  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t('or')}</span>
        <div style={{ flex: 1, height: 1, background: 'var(--border-soft)' }} />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        {hasGoogle && (
          <a
            href={`/api/auth/oauth/google?return=${encodeURIComponent(returnUrl)}`}
            className="btn"
            style={{
              width: '100%',
              display: 'flex',
        ...
        {hasGitHub && (
          <a
            href={`/api/auth/oauth/github?return=${encodeURIComponent(returnUrl)}`}
            className="btn"
            style={{
              width: '100%',
              display: 'flex',
